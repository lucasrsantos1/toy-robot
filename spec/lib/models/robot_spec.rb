...
RSpec.describe Robot do
  ...
  subject(:robot) { described_class.new(board: board)}
  ...
  describe '#report' do
    subject(:robot) { described_class.new(board: board, overrides: { reporter: FakeReporter })}
    ...
    context 'when passing a new reporter' do
      it 'prints report' do
        ...
        expect do
          robot.report
        end.to output("X: 1 Y: 1 Direction: NORTH\n").to_stdout
      end
    end
  end
end
...
class FakeReporter
  ...
  def report
    puts "X: #{@x} Y: #{@y} Direction: #{@direction.to_s}"
  end
end